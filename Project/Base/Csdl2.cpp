...
void CSDL2::SwapBuffer(RenderContext* renderContext)
{
	...
	//memcpy(mPixels, renderContext->depthBuffer, surface->pitch * surface->h);
...
void CSDL2::DrawRect(Rect* rect)
{
	SDL_Rect tmpRect = { rect->x, rect->y, rect->width, rect->height };
	SDL_RenderDrawRect(renderer, &tmpRect);    
}

void CSDL2::DrawCicle(Circle* circle)
{

}

void CSDL2::DrawLine(Line2d* line)
{
...
void CSDL2::Clean(RenderContext* renderContext, Color* color)
{
	...
	//std::fill(renderContext->depthBuffer, renderContext->depthBuffer + renderContext->width* renderContext->height, Mathf::NegativeInfinity);
